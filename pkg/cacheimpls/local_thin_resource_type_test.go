...
func TestResourceTypePKCacheKey_Key(t *testing.T) {
	k := ResourceTypePKCacheKey{PK: 1}
	assert.Equal(t, "1", k.Key())
}

func TestGetThinResourceType(t *testing.T) {
	expiration := 5 * time.Minute

	// valid
	retrieveFunc := func(key cache.Key) (interface{}, error) {
		return svctypes.ThinResourceType{}, nil
	}
	mockCache := memory.NewCache(
		"mockCache", false, retrieveFunc, expiration, nil)
	LocalThinResourceTypeCache = mockCache

	_, err := GetThinResourceType(1)
