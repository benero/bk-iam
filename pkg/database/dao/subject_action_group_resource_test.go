...
func Test_subjectActionGroupResourceManager_BulkDeleteBySubjectPKsWithTx(t *testing.T) {
	database.RunWithMock(t, func(db *sqlx.DB, mock sqlmock.Sqlmock, t *testing.T) {
		mock.ExpectBegin()
		mock.ExpectExec(`^DELETE FROM rbac_subject_action_group_resource WHERE subject_pk IN`).WithArgs(
			int64(1), int64(2),
		).WillReturnResult(sqlmock.NewResult(1, 1))
		mock.ExpectCommit()
...
func Test_subjectActionGroupResourceManager_DeleteByActionPKWithTx(t *testing.T) {
	database.RunWithMock(t, func(db *sqlx.DB, mock sqlmock.Sqlmock, t *testing.T) {
		mock.ExpectBegin()
		mock.ExpectExec(`^DELETE FROM rbac_subject_action_group_resource WHERE action_pk = (.*) LIMIT (.*)`).WithArgs(
			int64(1), int64(2),
