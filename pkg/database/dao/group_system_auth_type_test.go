...
func Test_groupSystemAuthTypeManager_ListByGroup(t *testing.T) {
	database.RunWithMock(t, func(db *sqlx.DB, mock sqlmock.Sqlmock, t *testing.T) {
		...
		FROM group_system_auth_type
		WHERE group_pk = (.*)`
		mockRows := sqlmock.NewRows([]string{"system_id", "group_pk", "auth_type"}).
			AddRow("1", int64(1), int(2)).
			AddRow("2", int64(1), int(3))
...
func Test_groupSystemAuthTypeManager_UpdateWithTx(t *testing.T) {
	database.RunWithMock(t, func(db *sqlx.DB, mock sqlmock.Sqlmock, t *testing.T) {
		...
		manager := &groupSystemAuthTypeManager{DB: db}
		rows, err := manager.UpdateWithTx(
			tx,
			GroupSystemAuthType{GroupPK: int64(1), AuthType: int64(2), SystemID: "system", Reversion: int64(1)},
		)
